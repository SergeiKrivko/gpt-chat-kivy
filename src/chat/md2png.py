...
class ImageExtension(markdown.Extension):
    ...
    def extendMarkdown(self, md):
        ...
        md.treeprocessors.register(self.treeprocessor, 'm2png', 1)
...
class ImageTreeprocessor(Treeprocessor):
    ...
    def __init__(self, width_spec, config=None):
        ...
        self.config = {
            "bold_font_path": r"C:\Windows\Fonts\calibrib.ttf",
            "blockquote_indent": 16,
            "code_indent": 16,
            "code_font_path": r"C:\Windows\Fonts\consola.ttf",
            "code_font_size": 20,
            "color": (255, 255, 255, 255),
            "default_font_path": r"C:\Windows\Fonts\calibri.ttf",
            "font_size": 20,
            "hr_color": (220, 220, 220, 255),
            "hr_padding": 0,
            "italics_font_path": r"C:\Windows\Fonts\calibrii.ttf",
    ...
    def handle_li(self, node):
        ...
        if list_type == "unordered":
            # Draw the bullet
            (w, h) = self.text_size("E", font=self.default_font)
            ...
            (w, h) = self.text_size(current_number, font=self.default_font)
            draw = self.ensure_image(h)
    ...
    def render_text(self, text, color, end_block=False, font=None):
        ...
        if self.in_pre:
            lines = text.split("\n")
            for line in lines:
                (w, h) = self.text_size(line, font=font)
                draw = self.ensure_image(h)
            ...
            while end_index < len(parts):
                w = 0
                while end_index < len(parts):
                    (w, h) = self.text_size(" ".join(parts[start_index:end_index + 1]),
                                           font=font, draw=draw)
    ...
    def text_size(self, text, font=None, draw=None):
        if draw is None:
            draw = self.image_draw
        x1, y1, x2, y2 = draw.textbbox((0, 0), text, font=font if font else self.default_font)
        return x2 - x1, y2 - y1
...
def md2png(md_str, width_spec, config) -> Image.Image:
    """
